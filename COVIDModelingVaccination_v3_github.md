...
for (i in 1:nrow(tracts_age_cleaned)){
  if (tracts_age_cleaned[i, "county_name"] %in% central_sierra_counties) {
      tracts_age_cleaned[i, "county_name"] <- "Central Sierra"
  }
  else if (tracts_age_cleaned[i, "county_name"] %in% north_ca_counties) {
      tracts_age_cleaned[i, "county_name"] <- "Northern California"
  }
  else if (tracts_age_cleaned[i, "county_name"] %in% north_sac_valley_counties) {
      tracts_age_cleaned[i, "county_name"] <- "Northern Sacramento Valley"
  }
  else if (tracts_age_cleaned[i, "county_name"] %in% greater_sac_counties) {
      tracts_age_cleaned[i, "county_name"] <- "Greater Sacramento"
  }
  else if (tracts_age_cleaned[i, "county_name"] %in% san_joaquin_val_counties) {
      tracts_age_cleaned[i, "county_name"] <- "San Joaquin Valley"
  }
  else if (tracts_age_cleaned[i, "county_name"] %in% santa_crz_san_benito) {
      tracts_age_cleaned[i, "county_name"] <- "SantaCruz_SanBenito"
  }
  else if (tracts_age_cleaned[i, "county_name"] %in% napa_sonoma) {
      tracts_age_cleaned[i, "county_name"] <- "Napa_Sonoma"
  }
}
```
...
merged_county_tract <- left_join(tracts_age_cleaned, merged_county, by=c("county_name" = "county_res")) %>%
  ...
  summarize(total_cases_by_age = round(sum(num_cases_by_tract_age)))
```
...
####################
#NOTE: CHANGE HERE
#total_cases_by_age or total_immune
####################
           incidence_rate_per_mo = total_cases_by_age/pop_2019/12,
...
![](COVIDModelingVaccination_v3_github_files/figure-gfm/unnamed-chunk-14-1.png)<!-- -->
...
![](COVIDModelingVaccination_v3_github_files/figure-gfm/unnamed-chunk-21-1.png)<!-- -->

``` r
#write.csv(homogeneous_and_targeted_estimated,"case_estimates.csv", row.names = FALSE)
homogeneous_and_targeted_estimated_with_diff <- homogeneous_and_targeted_estimated %>%
  mutate(absolute_diff = homogeneous-targeted_absolute,
         expanded_absolute_diff = homogeneous-targeted_absolute_expanded_age,
         incidence_diff = homogeneous - targeted_incidence,
         expanded_incidence_diff = homogeneous - targeted_incidence_expanded_age)
```
...
![](COVIDModelingVaccination_v3_github_files/figure-gfm/unnamed-chunk-23-1.png)<!-- -->
...
![](COVIDModelingVaccination_v3_github_files/figure-gfm/unnamed-chunk-29-1.png)<!-- -->
